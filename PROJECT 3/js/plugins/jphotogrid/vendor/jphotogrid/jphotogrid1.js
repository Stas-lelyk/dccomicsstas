...
(function() {
	jQuery.fn.jphotogrid = function(settings, callback) {
		...
		function hideSelected(callback){
			jQuery.container.find('.' + settings.selectedClass).each(function(){
		...
		return jQuery(this).each(function(){
			...
			jQuery(document).on('click', '.' + settings.activeClass, function(){
				select(this);
			});
			jQuery(document).on('click', '.' + settings.selectedClass, function(){
				hideSelected();
			});
			...
			jQuery(this).find('div')
				...
				.click(function(){
					jQuery(this).css('opacity', 0);
					jQuery.li = jQuery(this).parent();
					jQuery.li.css("z-index", 99);
					jQuery.li.animate({
						top: 0,
						left: 0,
						width: '100%',
